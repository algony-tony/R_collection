...
### 函数: scan
scan  也即是数据都有相同的类型, 要么都是数值, 要么都是字符串, 每个元素都用一个或多个空格分隔开, 或者通过参数 sep 指定分隔符. 如果 scan 的第一个参数是文件, 则从该文件中读取数据, 如果第一个参数是空白, 则从终端中读取数据, 以一个完全的空白行为结束符. scan 默认读取的是数值, 如果想要读取字符串则要指定 what="" 参数.
